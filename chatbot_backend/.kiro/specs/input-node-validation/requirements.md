...
- **Validation_Rule**: A constraint defined in an input node's config map that specifies how user input must be validated before acceptance
- **Validation_Service**: The component responsible for evaluating user input against the validation rules defined on an input node
...
- **Error_Message**: A text string sent to the user via WebSocket when their input fails validation, prompting them to try again
- **Node_Config**: The `config` map within a workflow node's JSON definition, containing node-specific settings like `variableName` and `nodeType`
...
3. WHEN a `validation` object is present in the Node_Config, THE Workflow_Engine SHALL recognize an optional `errorMessage` (string, maximum 500 characters) property that defines a custom error message indicating why validation failed
...
6. IF user input fails one or more validation rules on an Input_Node, THEN THE Workflow_Engine SHALL send an error response to the user via WebSocket containing the custom `errorMessage` if defined or a default message indicating which rule failed, and SHALL remain paused on the same Input_Node awaiting a new submission
...
1. WHEN `required` is set to `true` in the input node's Validation_Rule AND the user submits a reply that is null, empty (zero-length), or contains only whitespace characters, THEN THE Validation_Service SHALL reject the input and send a ChatErrorResponse to `/topic/chat/{sessionId}` indicating that the field is required, and the session shall remain paused at the current input node without storing the reply in Session_Context
...
1. IF `minLength` is defined in the Validation_Rule AND the trimmed user input character count is less than the `minLength` value, THEN THE Validation_Service SHALL reject the input and return a validation error message indicating the input is too short and specifying the minimum required length
2. IF `maxLength` is defined in the Validation_Rule AND the trimmed user input character count is greater than the `maxLength` value, THEN THE Validation_Service SHALL reject the input and return a validation error message indicating the input is too long and specifying the maximum allowed length
...
2. IF the user input does not match the defined `pattern`, THEN THE Validation_Service SHALL reject the input by sending a validation error response to the user indicating the expected format, SHALL NOT store the input in the session context, and SHALL keep the session paused on the current input node awaiting a new input
...
4. IF pattern matching execution exceeds 2 seconds for a single input evaluation, THEN THE Validation_Service SHALL terminate the match, log a warning, and accept the input (fail-open behavior to prevent ReDoS from blocking users)
...
1. WHEN `numericOnly` is set to `true` in the Validation_Rule AND the user input contains any character that is not an ASCII digit (0-9), THEN THE Validation_Service SHALL reject the input
...
2. WHEN a custom `errorMessage` is defined in the first failing Validation_Rule, THE Workflow_Engine SHALL use the custom error message text as the `error` field in the ChatErrorResponse
3. IF no custom `errorMessage` is defined in the failing Validation_Rule, THEN THE Workflow_Engine SHALL generate a default error message that includes the validation constraint type that failed (e.g., "Input must match the required format", "Input must not exceed N characters")
...
5. IF multiple Validation_Rules are defined on the current Input_Node, THEN THE Workflow_Engine SHALL evaluate them in their defined order and return the error message for the first failing rule
...
5. THE Validation_Service SHALL support a maximum of 5 simultaneous validation rules on a single Input_Node (required, minLength, maxLength, numericOnly, pattern)

### Requirement 8: Placeholder Support in Error Messages

**User Story:** As a workflow designer, I want to use placeholders in custom error messages, so that error messages can include dynamic values from the session context.

#### Acceptance Criteria

1. WHEN a custom `errorMessage` contains `{{variable}}` placeholders, THE Workflow_Engine SHALL resolve placeholders using the PlaceholderService and the current Session_Context before sending the message to the user
2. THE Workflow_Engine SHALL support the special `{{input}}` placeholder in error messages, which resolves to the user's current invalid input value (not stored in Session_Context)
3. IF a `{{variable}}` placeholder in the error message references a key that does not exist in the Session_Context, THEN THE Workflow_Engine SHALL replace that placeholder with an empty string
4. THE Workflow_Engine SHALL resolve placeholders in the error message AFTER determining that validation has failed, ensuring the message reflects the current session state at the time of failure
